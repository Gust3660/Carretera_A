...
conexiones: Dict[str, Dict[str, int]] = {
    ...
    'MORELOS': {'QRO': 524},
    'CDMX': {'Jiloyork': 125, 'QRO': 423, 'HGO': 491, 'dexcani el alto': 34},
...
def geodist(lat1: float, lon1: float, lat2: float, lon2: float) -> float:
    lat1r = radians(lat1)
    lon1r = radians(lon1)
    lat2r = radians(lat2)
    lon2r = radians(lon2)
...
def buscar_solucion_USC(estado_inicial: str, solucion: str) -> Optional[Nodo]:
    solucionado = False
...
def get_route(nodo_solucion: Nodo, estado_inicial: str) -> List[str]:
    resultado: List[str] = []
    nodo = nodo_solucion
...
def search_route(estado_inicial: str, solucion: str) -> Dict[str, Optional[object]]:
    nodo_solucion = buscar_solucion_USC(estado_inicial, solucion)
    if nodo_solucion is None:
        return {'route': None, 'cost': None}

    ruta = get_route(nodo_solucion, estado_inicial)
    return {'route': ruta, 'cost': nodo_solucion.get_costo()}
...
def get_coordinates_for_route(route: List[str]) -> List[Tuple[str, float, float]]:
    return [(node, coord[node][0], coord[node][1]) for node in route if node in coord]
